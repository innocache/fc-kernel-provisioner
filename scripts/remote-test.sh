...
ssh -f "$HOST" "cd $REMOTE_DIR && nohup sudo uv run python -m fc_pool_manager.server --config config/fc-pool.yaml --socket /var/run/fc-pool.sock -v </dev/null >/tmp/fc-pool-manager.log 2>&1"
...
ssh "$HOST" "
    if ! command -v caddy &>/dev/null; then
        ...
        sudo curl -sS 'https://caddyserver.com/api/download?os=linux&arch=amd64' -o /usr/local/bin/caddy
