...
class PoolManager:
    ...
    async def _boot_vm(self) -> VMInstance:
        ...
        tap_name = self._network.tap_name(short_id)
        mac = self._network.mac_from_ip(ip)
        ...
        async with self._boot_lock:
            while (
                ...
                try:
                    ...
                    break
                # If _boot_vm didn't add a VM (e.g., mock in tests), stop looping
                if self.total_count == prev_total:
                    break
