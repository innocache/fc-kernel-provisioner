...
def make_kernel_spec(pool_socket="/var/run/fc-pool.sock", vcpu_count=1, mem_size_mib=512):
    spec = KernelSpec()
    spec.metadata["kernel_provisioner"] = {
        "config": {
            "pool_socket": pool_socket,
...
class TestFirecrackerProvisioner:
    ...
    def provisioner(self):
        ...
        p = FirecrackerProvisioner(kernel_id="test-kernel", kernel_spec=spec)
        p.connection_info = {
            "key": "test-hmac-key",
            "ip": "127.0.0.1",
            "transport": "tcp",
